...
from Iremulbnet import iremulbnet
...
def main():
    ...
    print("using {} images for training, {} images for validation.".format(train_num,
                                                                           val_num))
    net = iremulbnet(num_classes=100).to(device)
    ...
    torch.cuda.synchronize()
    endg = time.time()
    print('训练时间: %.3f ' % (endg-startg))
    
    fil=open("iremulbnet.txt","w")
